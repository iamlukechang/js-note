 ...
 * 01 Knapsack Problem
 *
 * Given a pack and a bounch items(the available number of each item is only 1).
 * The pack volume is limited, and each item has different value. How to select
 * items and put them in the pack to get the maximum value?
 *
 * Time: 
 *   O(NW)
 *
 ...
 *        move the target to the previous one, too
 *
 ...
 * Example:
 *
 *   knapsack([{value: 2, volume: 1}, {value: 3, volume: 2}], 1); // 2
 *
...
function knapsack(itemArr, volume, itemIdx, cache) {
  ...
  cache[volume] = {
    value: Math.max(
      ...
      // step 3.2
      knapsack(itemArr, volume - targetItem.volume, itemIdx - 1, cache) +
          targetItem.value
    ),
    itemIdx: itemIdx 
  };
